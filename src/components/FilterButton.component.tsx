...
const FilterButton: FC<IFilterButtonProps> = ({ checked, label, name, onChange, value }) => {
	...
	return (
		<label className='cursor-pointer' htmlFor={id}>
			...
			<button className='bg-neutral-0 peer-checked:text-neutral-0 pointer-events-none h-[36px] rounded-full px-[18px] shadow-sm peer-checked:bg-red-700'>{label}</button>
		</label>
	);
};
